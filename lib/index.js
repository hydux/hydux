        ...
        var _loop_1 = function (key) {
            ...
            if (utils_1.isFn(subFrom)) {
                actions[key] = function (msgData) {
                    ...
                    try {
                        _a = runAction(subFrom, msgData, state, actions), nextState = _a[0], cmd = _a[1];
                    }
                    catch (error) {
                        console.error(error);
                        onError(error);
                ...
                init(state[key] || (state[key] = {}), (actions[key] = subFrom.constructor ? new subFrom.constructor() : {}), subFrom, path.concat(key));
            }
        };
        for (var key in from) {
            _loop_1(key);
