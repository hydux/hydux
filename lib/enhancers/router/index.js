...
function parseNestedRoutes(routes) {
    function rec(routes, newRoutes) {
        newRoutes[routes.path] = __assign({}, routes, { children: [] });
        routes.children
            .map(function (r) { return (__assign({}, r, { path: routes.path.replace(/\/$/, '') + '/' + r.path.replace(/^\//, ''), action: r.action, parents: (routes.parents || []).concat(__assign({}, routes, { parents: [], children: [] })), children: r.children })); })
            .forEach(function (r) {
            rec(r, newRoutes);
        });
        return newRoutes;
