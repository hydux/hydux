...
const Counter = {
  init: () => ({ count: 1 }),
  actions: {
    up: () => state => (state.count += 1, state),
    upN: (n: number) => (state: CounterState, actions: CounterActions) => (state.count += n, state),
    down: () => state => (state.count -= 1, state),
    reset: () => state => (state.count = 1, state),
...
describe('mutable core api', () => {
  ...
  it('actions', () => {
    ...
    ctx = app<any, any>({
      init: () => ({ count: 1 }),
      actions: {
        up: _ => state => (state.count += 1, state),
        down: _ => state => (state.count -= 1, state),
        reset: _ => state => (state.count = 1, state),
      },
