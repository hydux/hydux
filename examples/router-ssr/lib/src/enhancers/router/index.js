...
export default function withRouter(props) {
    ...
    return function (app) { return function (props) {
        ...
        var ctx = app(__assign({}, props, { init: function () {
            ...
            }), props.subscribe ? props.subscribe(state) : Cmd.none); }, actions: __assign({}, props.actions, (_a = { history: {
                ...
                if (loc.template) {
                    var action = routesMap[loc.template];
                    var _a = runAction(action(loc), state, actions), nextState = _a.state, cmd = _a.cmd;
...
export function parseNestedRoutes(routes) {
    function rec(routes, newRoutes) {
        ...
        children
            ...
            var action = r.action;
            if (!action && r.update) {
                if (!r.key || !r.component) {
                    error('router', 'invalid router config, r.update must use with r.key and r.component', r, routes);
                }
                action = function (loc) { return function (state, actions) {
                    var ret = normalize(r.update(loc, state, actions), state);
                    return {
                        state: ret.state,
                        cmd: Cmd.batch(ret.cmd, Cmd.map(function (_) { return _[r.key]; }, normalize(r.component.init()).cmd))
                    };
                }; };
            }
            return __assign({}, r, { path: join(routes.path, r.path), action: action, parents: (routes.parents || []).concat(__assign({}, routes, { parents: void 0, children: void 0 })), children: r.children });
        })
